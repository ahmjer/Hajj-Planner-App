...
# الدوال المساعدة للحساب (Logic) - تم مراجعة الأقواس داخل الدوال
...
def distribute_staff(total_basic_staff, ratio_supervisor, ratio_assistant_head, ratio_head):
    ...
    # تم التأكد من سلامة الأقواس المعقوفة وعلامات التنصيص هنا
    return {
        'رئيس': رئيس, 
...
# تحديد الإدارات وتصنيفها - تم التأكد من إغلاق جميع الأقواس المعقوفة
DEPARTMENTS = {
    "مراكز الضيافة": [
...
# تم التأكد من إغلاق القوس لكل عنصر في الشريط الجانبي
...
for dept in DEPARTMENTS[department_type_choice]:
    ...
    # تم التأكد من سلامة إغلاق F-string والقوس
    elif dept['type'] == 'Time':
...
if calculate_button: 
    ...
    # 📌 التتبع: تأكيد وصول الكود إلى هنا
    ...
    for dept, ratio in ratios.items():
        ...
        # تم التأكد من إغلاق جميع سلاسل النصوص والأقواس المعقوفة
        all_results.append({
            'الإدارة': dept, 
    ...
    for dept, time_min in time_based_inputs.items():
        res_basic_time = calculate_time_based_staff(num_hajjaj * 2, time_min, service_days
