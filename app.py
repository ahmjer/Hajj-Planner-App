...
# تحديد الإدارات وتصنيفها - تم استخدام أسماء إنجليزية لتجنب أخطاء الترميز في النقل
DEPARTMENTS = {
    "مراكز الضيافة": [
...
# ... (باقي الكود كما كان سابقاً)
# ... [المدخلات والمقاييس]
# ... [القسم الثاني: مدخلات الإدارات]
...
for dept in DEPARTMENTS[department_type_choice]:
    ...
    if dept['type'] == 'Ratio':
        ...
        ratios[name] = st.sidebar.number_input(label, min_value=1, value=dept['default_ratio'], key=f"{name}_ratio_input")
    
    elif dept['type'] == 'Time':
        label = f"{name} (دقيقة/حاج)"
        time_based_inputs[name] = st.sidebar.number_input(label, min_value=0.5, value=dept['default_time'], step=0.1, key=f"{name}_time_input")
        ...
        bus_inputs['Ratio'] = st.sidebar.number_input(label, min_value=1, value=dept['default_ratio'], key=f"{name}_ratio_val")
