...
DEFAULT_HEAD_ASSISTANT_RATIO = 3
...
def to_excel(df):
    ...
    # تأكد من إغلاق الكاتب لحفظ البيانات
    writer.close() 
    processed_data = output.getvalue()
    return processed_data

# تعريف شامل لجميع فروع الإدارات لسهولة الوصول إليها في وضع الحساب الموحد
...
# الثوابت الخاصة بالإدارات (لا تغيير)
DEPARTMENTS = {
    "الضيافة": [
...
def all_departments_page(sidebar_inputs):
    """
    شاشة لعرض وحساب احتياج القوى العاملة لجميع الإدارات في جدول واحد.
    """
    st.title("📊 تخطيط القوى العاملة الموحد")
    st.markdown("---")
    
    # 1. تعريف المتغيرات من الشريط الجانبي
    ...
    # 2. منطقة الإعدادات والمعايير (اختياري: يمكن وضعها في الشريط الجانبي لتجنب التكرار)
    st.subheader("إعدادات المعايير لكافة الإدارات")
    st.warning("⚠️ **ملاحظة:** يتم استخدام القيم الافتراضية للمعايير (النسب والتغطية والوقت) لجميع الإدارات هنا. للإدخال المفصل لكل إدارة، يرجى العودة للشاشة الرئيسية.")
    
    # 3. جدول عرض النتائج الموحد
    all_results = []
    ...
    # عملية الحساب لجميع الإدارات
    for dept_name, dept_info in ALL_DEPARTMENTS_FLAT.items():
        
        dept_type = dept_info['type']
        criterion = dept_info.get('default_criterion', 'Present') # استخدام المعيار الافتراضي
        coverage = dept_info.get('default_coverage', 100) / 100 # استخدام التغطية الافتراضية
        
        res_basic = 0
        
        # أ. حساب الإدارات المعتمدة على النسبة (Ratio)
        if dept_type == 'Ratio':
            ratio = dept_info['default_ratio']
        ...
        # ب. حساب إرشاد الحافلات (Bus_Ratio)
            ...
            # نعتبر قيمة افتراضية لعدد الحافلات في هذا الوضع الموحد لغرض العرض
            num_units = 20 # يمكن أن تكون مدخل عام في الشريط الجانبي
            bus_ratio = dept_info['default_ratio']
            res_basic = calculate_ratio_based_staff(num_units, bus_ratio)
            
        # ج. حساب الإدارات المعتمدة على الزمن (Time-based)
        elif dept_type == 'Time':
            time_min = dept_info['default_time']
            num_hajjaj_for_dept = hajjaj_data[criterion]
            actual_hajjaj_in_center = num_hajjaj_for_dept * coverage
            # افتراض الضرب في 2 لتغطية الوصول والمغادرة (كما في الكود الأصلي)
            res_basic = calculate_time_based_staff(actual_hajjaj_in_center * 2, time_min, service_days, staff_work_hours_day)
    ...
    st.success("✅ تم الانتهاء من حساب الاحتياج الموحد لجميع الإدارات.")
    
    # عرض النتائج
    st.subheader("جدول الاحتياج الموحد")
...
# -------------------------------------------------------------------
# الدالة الرئيسية للشاشة الافتراضية (Main Page Logic)
# -------------------------------------------------------------------
def main_page_logic():
    # العنوان الرئيسي
    ...
    with st.container(border=True):
        ...
        # يجب تعريف هذه المتغيرات قبل استخدامها في الحساب
        ratios = {}
        time_based_inputs = {}
        bus_ratio_inputs = {}
    ...
    if calculate_button:
        ...
        # جلب مدخلات الشريط الجانبي المستخدمة في هذه الصفحة
        ...
        for dept, time_min in time_based_inputs.items():
            ...
            # ** ملاحظة: الضرب في 2 لسبب غير واضح في الكود الأصلي. (يفترض لتغطية الوصول والمغادرة) **
...
if st.sidebar.button("1. وضع الحساب الفردي (الإدارة المختارة)", key="go_to_main_page"):
    st.session_state['current_page'] = 'main'
    st.rerun()

if st.sidebar.button("2. وضع الحساب الموحد (جميع الإدارات)", type="primary", key="go_to_all_page"):
...
if st.session_state['current_page'] == 'main':
    ...
    # تجميع المدخلات الضرورية للشريط الجانبي
    sidebar_inputs = {
        'num_hajjaj_present': st.session_state["num_hajjaj_present"],
        'num_hajjaj_flow': st.session_state["num_hajjaj_flow"],
        'service_days': st.session_state["service_days"],
    ...
    all_departments_page(sidebar_inputs)
