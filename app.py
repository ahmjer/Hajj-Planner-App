...
# 📌 دالة توزيع الموظفين المعدلة لقبول عدد مساعدي الرئيس الإلزامي
def distribute_staff(total_basic_staff, ratio_supervisor, shifts, required_assistant_heads=0, ratio_assistant_head=DEFAULT_HEAD_ASSISTANT_RATIO):
    """
    توزع موظفي الخدمة على الهيكل الإداري مع مراعاة العدد الإلزامي لمساعدي الرئيس.
    """
    ...
    # *** منطق حساب مساعد الرئيس المعدل ***
    if required_assistant_heads > 0:
        # إذا تم تحديد عدد إلزامي، يتم ضربه في عدد الورديات لضمان التغطية الثابتة
        assistant_head_fixed = required_assistant_heads * shifts 
        # يتم أخذ الحد الأقصى بين العدد الثابت والحساب الهرمي
        assistant_head = max(assistant_head_fixed, math.ceil(total_supervisors / ratio_assistant_head))
    else:
        # إذا كان العدد الإلزامي صفر، لا يتم تعيين مساعد رئيس
        assistant_head = 0
        
    head = 1 # نفترض أن دور الرئيس دائماً مطلوب
...
def switch_to_main():
    ...
    st.session_state['run_calculation_all'] = False # إيقاف مؤشر الحساب الموحد
...
def all_departments_page():
    ...
    with st.container(border=True):
                ...
                for i, dept in enumerate(depts):
                    ...
                    if name not in user_settings:
                        user_settings[name] = {
                            ...
                            'bus_count': 20, 
                            'events_multiplier': 2,
                            'required_assistant_heads': 0 # 📌 القيمة الافتراضية لمساعد الرئيس
                        }
        ...
        if calculate_button:
            ...
            st.session_state['run_calculation_all'] = True # مؤشر لبدء الحساب
            st.rerun() 
    ...
    if st.session_state.get('run_calculation_all', False):
        ...
        for dept_name, dept_info in ALL_DEPARTMENTS_FLAT.items():
            ...
            # ... (منطق حساب res_basic هنا كما هو معتمد على settings)
            ...
            required_assistant_heads = settings['required_assistant_heads'] # 📌 استخدام العدد المخصص
...
def main_page_logic():
    ...
    # 📌 إضافة مدخل مساعد الرئيس الإلزامي في هذه الشاشة
    ...
    if calculate_button:
        ...
        ratio_supervisor = st.session_state["ratio_supervisor"]
        ratio_assistant_head = st.session_state["ratio_assistant_head"]
        required_assistant_heads_per_shift = st.session_state["required_assistant_heads_main"] # 📌 جلب المدخل
        ...
        for dept, ratio in ratios.items():
            ...
            # 📌 تطبيق الهيكل الإداري باستخدام المدخل الجديد
            staff_breakdown = distribute_staff(
                res_basic, 
                ratio_supervisor, 
                shifts_count, 
        ...
        # ... (عرض النتائج والإجمالي كما كان سابقاً)
...
# 📌📌📌 كتلة CSS (لضمان RTL وتصميم ثابت) 📌📌📌
st.markdown("""
<style>
/* ... (كود CSS كما كان سابقاً) ... */
...
[data-testid="stSidebarContent"] { direction: rtl; text-align: right; visibility: hidden; }
[data-testid="stSidebarUserContent"] { visibility: visible !important; }
</style>
