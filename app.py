...
DEPARTMENTS = {
    ...
    "الدعم والمساندة": [
        ...
        {'name': 'الزيارة وإرشاد التأهيل', 'type': 'Time', 'default_time': 2.5}, # تم تحويلها لزمني
...
# المدخل الجديد لاختيار نوع الإدارة
...
ratios = {} # إدارات التغطية
time_based_inputs = {} # إدارات الزمن
bus_ratio_inputs = {} # إرشاد الحافلات (للتغطية بالعدد الخاص)
...
if calculate_button: 
    ...
    # ب. حساب إرشاد الحافلات (معيار خاص) - إذا كانت ضمن النوع المختار
    if 'Bus_Count' in bus_ratio_inputs:
        num_units = bus_ratio_inputs['Bus_Count']
        bus_ratio = bus_ratio_inputs['Ratio']
        
        # نستخدم num_units (عدد الحافلات) بدلاً من num_hajjaj للحساب الأساسي
    ...
    for dept, time_min in time_based_inputs.items():
        # عدد الأحداث = عدد الحجاج * 2 (وصول ومغادرة)
    ...
    # نستخدم عرض البيانات العادي لتجنب الحاجة إلى matplotlib
    ...
    st.info(f"يرجى تعديل المدخلات في الشريط الجانبي ثم النقر على زر الحساب لرؤية النتائج لـ {department_type_choice}.")
